...
if __name__ == "__main__":
    ...
    chatbot = chatbot_estimator.ChatBotTrainer(subword_processor, chatbot_model, model.TransformerOptimizer(0.001), load_from_checkpoint = True)
    ...
    # %%
    try:
        chatbot.fit(train_pipeline, train_pipeline, inference_pipeline, steps_per_epoch = 2000, debugging = False, checkpoint_every=5)
    except Exception as err:
        print('Caught execption >:(\nSaving model...')
        chatbot.checkpoint_manager.save()
        raise(err)
