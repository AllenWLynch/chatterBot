...
def transform(conversation_obj):
    ...
    data['encoded_tweet'] = [preprocess_text.preprocess(tweet) for tweet in data['tweets']]
    data['encoded_response'] = preprocess_text.preprocess(data['response'])

    data['sender_ids'] = [
        sender_id
        for sender, tweet in zip(data['sender'],data['encoded_tweet']) 
        for sender_id in [binarize_author_id(preprocess_text.encode_authors(sender))] * len(tweet) 
    ]
